...
const PALABRAS_POR_SEGUNDO = 2.3;
const MAX_CARACTERES_GUION = 3000;

const VOCES_DISPONIBLES = [
    { id: 'es-CO-GonzaloNeural', label: 'Hombre · Colombiano (Gonzalo)', estilo: 'Profesional, ideal para Caicedonia' },
    { id: 'es-CO-SalomeNeural', label: 'Mujer · Colombiana (Salome)', estilo: 'Cercana y amigable' },
    { id: 'es-MX-JorgeNeural', label: 'Hombre · Neutro latino (Jorge)', estilo: 'Neutral, claro' },
    { id: 'es-ES-AlvaroNeural', label: 'Hombre · Formal (Álvaro)', estilo: 'Corporativo, tono serio' },
];

const FPS_POR_PLANTILLA = {
    ReelAraStyle: 30,
    ReelCinematico: 30,
    TutorialReel: 30,
    OfferWonReel1: 30,
...
function obtenerVozEdge(voiceId) {
    if (process.env.EDGE_TTS_VOICE) return process.env.EDGE_TTS_VOICE;
    const voz = VOCES_DISPONIBLES.find((v) => v.id === voiceId);
    return voz ? voz.id : 'es-CO-GonzaloNeural';
}

function calcularFramesVideo(plantillaId, numProductos) {
    const n = Math.max(1, numProductos || 1);
    switch (plantillaId) {
        case 'ReelBrutalismo':
            return n * 90 + 90;
        case 'ReelAraStyle':
...
function estimarDuracionVideoSeg(plantillaId, numProductos) {
    const fps = FPS_POR_PLANTILLA[plantillaId] || 60;
    return calcularFramesVideo(plantillaId, numProductos) / fps;
}
...
function estimarSegundosPorPalabras(palabras) {
    return Math.round((palabras / PALABRAS_POR_SEGUNDO) * 10) / 10;
}

function estimarPalabrasRecomendadas(segundosVideo) {
    return Math.max(8, Math.floor(segundosVideo * PALABRAS_POR_SEGUNDO));
}

function estimarGuionParaVideo(plantillaId, numProductos, textoGuion) {
    const duracionVideoSeg = estimarDuracionVideoSeg(plantillaId, numProductos);
    const palabrasRecomendadas = estimarPalabrasRecomendadas(duracionVideoSeg);
    const palabrasActuales = contarPalabras(textoGuion);
    const segundosEstimados = estimarSegundosPorPalabras(palabrasActuales);
    const excedeVideo = palabrasActuales > palabrasRecomendadas;

    return {
        plantillaId,
        numProductos,
        duracionVideoSeg: Math.round(duracionVideoSeg * 10) / 10,
        palabrasRecomendadas,
        palabrasActuales,
        segundosEstimados,
        excedeVideo,
        palabrasPorSegundo: PALABRAS_POR_SEGUNDO,
        nota: excedeVideo
            ? 'El guion supera la duración del video. El reel se alargará y la imagen puede quedarse fija al final.'
            : 'Guion dentro del tiempo recomendado.',
    };
}
...
function construirGuionVoz(productosProcesados, titulo) {
    const tituloFinal = (titulo || 'ofertas imperdibles').trim();
    const intro = `Caicedonia, ${tituloFinal} en Surtitodo Ideal. `;

    let cuerpo = '';
    if (productosProcesados.length === 0) {
        cuerpo = 'Aprovecha precios especiales en mercado, lácteos y más.';
    } else if (productosProcesados.length === 1) {
        const p = productosProcesados[0];
        cuerpo = `${p.nombre} a ${p.precioFmt} pesos.`;
    } else {
        cuerpo = productosProcesados
            .map((p) => `${p.nombre} a ${p.precioFmt} pesos`)
            .join('. ') + '.';
    }

    const cierre =
        ' Pide en surtitodoideal.com o WhatsApp tres doce ochocientos cuarenta sesenta y tres doce.';
...
async function generarVozGratis(guion, remotionPublicDir, voiceId) {
    ...
    const vozEdge = obtenerVozEdge(voiceId);

    const tts = new MsEdgeTTS();
    await tts.setMetadata(vozEdge, OUTPUT_FORMAT.AUDIO_24KHZ_48KBITRATE_MONO_MP3);
    const { audioStream } = await tts.toStream(texto.slice(0, MAX_CARACTERES_GUION));
...
module.exports = {
    ...
    estimarDuracionVideoSeg,
    estimarGuionParaVideo,
    contarPalabras,
};
